...
export async function getTelegramKPI({ date } = {}) {
  ...
  const [rows] = await analyticsPool.query(
    `SELECT
     ...
     FROM telegram_daily_stats WHERE stat_date = ?`,
    [d]
  );
  const r = rows[0] || {};
  return {
    jamiMurojaat:      r.jami           || 0,
    javobBerilgan:     r.javob_berilgan  || 0,
    javobKutilayotgan: r.kutilayotgan    || 0,
    ortachaJavobDaq:   String(r.avg_daq    || '0'),
...
export async function getMessageCategories({ date } = {}) {
  ...
  const [rows] = await analyticsPool.query(
    `SELECT
       is_answered,
       COUNT(*) AS v,
       ROUND(AVG(response_minutes),1) AS avg_daq
     FROM telegram_response_details
     WHERE stat_date = ?
     GROUP BY is_answered`,
    [d]
  );
  const answered = rows.find(r => r.is_answered) || { v: 0 };
  const waiting  = rows.find(r => !r.is_answered) || { v: 0 };
  return [
