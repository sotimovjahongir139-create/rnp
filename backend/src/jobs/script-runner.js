...
for (const id of Object.keys(REGISTRY)) {
  _state.set(id, { status: 'idle', lastRun: null, nextRun: null, exitCode: null, logs: [], pid: null });
}
...
export function getStatus() {
  return Object.values(REGISTRY).map((cfg) => {
    ...
    return { id: cfg.id, name: cfg.name, description: cfg.description, file: cfg.file,
      exists: fs.existsSync(fp), status: s.status, lastRun: s.lastRun,
      nextRun: s.nextRun, exitCode: s.exitCode, pid: s.pid };
  });
}
...
export function runScript(id) {
  ...
  proc.on('close', (code) => {
    s.pid = null; s.exitCode = code; s.status = code === 0 ? 'success' : 'error';
    _push(id, `[${new Date().toISOString()}] <<< Tugadi: exit=${code}`);
    logger.info(`[ScriptRunner] ${id} completed exit=${code}`);
  });
  proc.on('error', (err) => {
    s.pid = null; s.status = 'error'; s.exitCode = -1;
...
export function runAllScripts() {
  return Object.keys(REGISTRY).map((id) => ({ id, result: runScript(id) }));
}
